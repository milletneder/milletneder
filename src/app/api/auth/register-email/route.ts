...
import { isEmailOtpVerified, clearEmailVerified } from '@/lib/auth/phone-otp-store';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, city, district, party, password, referralCode, fingerprint, recoveryEmail } = body;
    ...
    if (!isEmailOtpVerified(trimmed)) {
      return NextResponse.json({ error: 'E-posta adresi doğrulanmamış. Önce OTP ile doğrulayın.' }, { status: 401 });
    }
    ...
    if (existingUser) {
      ...
      if (isIncomplete) {
        ...
        return NextResponse.json({ error: 'Bu e-posta ile zaten kayıtlı bir hesap var.' }, { status: 409 });
      }
    } else {
      result = await registerNewUser(input);
    }

    console.log(`[REGISTER-EMAIL] Registration result:`, JSON.stringify(result, null, 2).slice(0, 500));
    ...
    clearEmailVerified(trimmed);

    return NextResponse.json(result, { status: 201 });
  } catch (error) {
    console.error('Register email error:', error);
    await logAuthEvent({ eventType: 'register_fail', authMethod: 'email', request, errorCode: 'server_error', errorMessage: String(error) });
