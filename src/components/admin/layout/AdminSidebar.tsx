...
function NavUser({ name, email, onLogout }: { name: string; email: string; onLogout?: () => void }) {
...
export default function AdminSidebarComponent({ adminName, adminEmail, onLogout }: AdminSidebarProps) {
  ...
  function renderNavGroup(label: string, items: typeof navMain) {
    return (
      <SidebarGroup>
  ...
  return (
    <Sidebar collapsible="offcanvas" variant="inset">
      <SidebarHeader>
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton asChild className="data-[slot=sidebar-menu-button]:!p-1.5 [&_svg]:!size-auto">
              <Link href="/admin">
                <svg width="100" height="14" viewBox="0 0 71 10" fill="none" xmlns="http://www.w3.org/2000/svg" className="shrink-0">
      ...
      <SidebarContent>
        {renderNavGroup('Ana', navMain)}
        {renderNavGroup('Veri Yönetimi', navData)}
        {renderNavGroup('Sistem', navSystem)}
      </SidebarContent>

      <SidebarFooter>
        <NavUser name={adminName || 'Admin'} email={adminEmail || ''} onLogout={onLogout} />
      </SidebarFooter>
    </Sidebar>
  );
}
