...
export default function Header({ totalVotes, daysRemaining, currentMonth, onVoteClick, userHasVoted }: HeaderProps) {
  ...
  // Dropdown disi tiklaninca kapat
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (loginFormRef.current && !loginFormRef.current.contains(e.target as Node)) {
    ...
    <header className="fixed top-0 left-0 right-0 z-40 bg-white border-b border-neutral-100">
      <div className="max-w-screen-2xl mx-auto px-6 h-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <Link href="/" className="flex items-center">
            ...
            <span className="ml-1.5 text-[9px] font-bold text-neutral-400 border border-neutral-300 px-1.5 py-0.5 leading-none uppercase tracking-wider">Beta</span>
          </Link>
          <div className="hidden sm:flex items-center gap-4 text-[11px] text-neutral-400">
            {currentMonth && <span>{currentMonth}</span>}
            {totalVotes !== undefined && (
              <>
                {currentMonth && <span className="text-neutral-200">|</span>}
                <span className="text-black"><Counter value={totalVotes} className="text-black text-[11px]" /> oy</span>
              </>
            )}
        ...
        <div className="hidden md:flex items-center gap-4">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} className="text-xs font-medium text-neutral-500 hover:text-black transition-colors">{link.label}</Link>
          ))}
          <a href="https://github.com/milletneder/milletneder" target="_blank" rel="noopener noreferrer" className="text-neutral-400 hover:text-black transition-colors" title="GitHub">
          ...
          {onVoteClick && (
            <button onClick={onVoteClick} className="border border-black bg-black text-white px-3 py-1 text-xs font-medium hover:bg-neutral-800 hover:border-neutral-800 transition-colors">
          ...
          {isLoggedIn ? (
            <div className="relative" ref={accountMenuRef}>
              <button onClick={() => setShowAccountMenu(!showAccountMenu)} className="text-xs font-medium text-black border border-neutral-200 px-3 py-1 hover:border-black transition-colors">Hesabım</button>
              {showAccountMenu && (
                <div className="absolute right-0 top-full mt-2 w-44 bg-white border border-neutral-200 shadow-xl z-50">
                  <Link href="/profil" onClick={() => setShowAccountMenu(false)} className="block px-4 py-2.5 text-xs text-neutral-700 hover:bg-neutral-50 transition-colors">Hesabıma Git</Link>
                  <div className="border-t border-neutral-100" />
                  <button onClick={() => { handleLogout(); setShowAccountMenu(false); }} className="block w-full text-left px-4 py-2.5 text-xs text-neutral-400 hover:text-red-600 hover:bg-neutral-50 transition-colors">Çıkış Yap</button>
                </div>
              )}
            </div>
          ) : (
            <div className="relative" ref={loginFormRef}>
              <button onClick={() => setShowLoginForm(!showLoginForm)} className="text-xs font-medium text-black border border-black px-3 py-1 hover:bg-black hover:text-white transition-all">Giriş</button>
              {showLoginForm && (
                <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-neutral-200 shadow-xl p-4 z-50">
                  <div className="flex border border-neutral-200 mb-4">
                    <button
                      onClick={() => { setLoginMethod('email'); setLoginError(''); }}
                      className={`flex-1 py-2 text-xs font-medium transition-colors ${loginMethod === 'email' ? 'bg-black text-white' : 'bg-white text-neutral-500 hover:text-black'}`}
                    >
                      E-posta
                    </button>
                    <button
                      onClick={() => { setLoginMethod('phone'); setLoginError(''); }}
                      className={`flex-1 py-2 text-xs font-medium transition-colors ${loginMethod === 'phone' ? 'bg-black text-white' : 'bg-white text-neutral-500 hover:text-black'}`}
                    >
                      Telefon
                    </button>
                  </div>
                  ...
                  />
                  {loginLoading && <p className="text-neutral-400 text-xs mt-2 text-center">Giriş yapılıyor...</p>}
                </div>
              )}
            </div>
          )}
        ...
        {/* Mobile hamburger */}
        <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden flex flex-col gap-[5px] p-1" aria-label="Menu">
          <span className={`block w-5 h-[2px] bg-black transition-transform ${mobileMenuOpen ? 'rotate-45 translate-y-[7px]' : ''}`} />
          <span className={`block w-5 h-[2px] bg-black transition-opacity ${mobileMenuOpen ? 'opacity-0' : ''}`} />
          <span className={`block w-5 h-[2px] bg-black transition-transform ${mobileMenuOpen ? '-rotate-45 -translate-y-[7px]' : ''}`} />
        </button>
      </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-neutral-100 px-6 py-4 space-y-3">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} onClick={() => setMobileMenuOpen(false)} className="block text-sm font-medium text-black">{link.label}</Link>
          ))}
          {isLoggedIn && (
            <Link href="/profil" onClick={() => setMobileMenuOpen(false)} className="block text-sm font-medium text-black">Hesabım</Link>
          )}
          <a href="https://github.com/milletneder/milletneder" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm font-medium text-neutral-500">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/></svg>
            GitHub
          </a>
          ...
          <div className="pt-2 border-t border-neutral-100">
            {isLoggedIn ? (
              <button onClick={handleLogout} className="text-sm text-neutral-400 hover:text-red-600 transition-colors">Çıkış Yap</button>
            ) : (
          ...
          {onVoteClick && (
            <button onClick={() => { onVoteClick(); setMobileMenuOpen(false); }} className="w-full border border-black bg-black text-white py-2 text-xs font-medium hover:bg-neutral-800 transition-colors">
              {userHasVoted ? 'Oy Değiştir' : 'Oy Ver'}
            </button>
          )}
        </div>
